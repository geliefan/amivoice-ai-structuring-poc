...
## アクション前処理レイヤー（音声入力をすぐIssue化しない）

このPoCの中心は、**文字起こし結果をすぐIssue化せず、「アクション化してよいか」を
先に判定する前処理レイヤー**です。Issueを作る機能ではなく、
**Issue化してはいけないケースを止める機能**を重視しています。

Issue Modeで構造化すると、出力の冒頭に必ず「判定」ブロックが出ます。

- **推奨アクション**: `Issue化` / `調査メモ` / `質問テンプレ` / `Runbook候補` /
  `Backlog候補` / `保存のみ` / `何もしない` のいずれか1つ。
- **判断理由**: なぜそのアクションを推奨するか。
- **Issue化を止める理由 ／ してよい理由**: 止める場合は何が不足しているか。

判定の考え方:

- 再現条件・期待結果・実際の結果・影響範囲がある程度揃っていれば `Issue化`。
- 現象はあるが情報が不足していれば `調査メモ`、主観的な違和感・愚痴が中心なら `保存のみ`。
- **文字起こし信頼度が「低」、または技術用語の誤認識候補がある場合は、
  確定情報として扱わず安易にIssue化を推奨しない**（生成AIに綺麗な嘘のIssueを
  作らせない）。

`/convert` 画面では、この判定をバッジ・理由とともに表示し、
「このままIssue化 / 調査メモとして扱う / 確認事項だけコピー / 追加で話す」という
**人間が最終判断するための選択肢**を提示します（外部Issue登録は行いません）。

API（`/api/structure`・`/api/transcribe-and-structure`）のレスポンスにも、
この判定が `triage` フィールド（`recommendedAction` / `actionLabel` /
`actionRationale` / `blockedReason` / `shouldCreateIssue`）として含まれます。
判定ロジックの実装は [src/lib/llm/actionTriage.ts](./src/lib/llm/actionTriage.ts)、
プロンプトは [src/lib/prompts/issuePrompt.ts](./src/lib/prompts/issuePrompt.ts)
を参照してください。`/samples` には、Issue化してよい例・止める例（STOP）・
誤認識で保留する例（HOLD）を用意しています。
...
### AmiVoice token-level confidence による低信頼語句の抽出

AmiVoiceの同期HTTPレスポンスは、発話全体の信頼度
（`results[].confidence`）に加えて、**token単位**の情報を返します。

```jsonc
{
  "results": [
    {
      "confidence": 0.93,            // 発話全体（高くても局所的に低い語がある）
      "tokens": [
        { "written": "リフィル", "spoken": "りふぃる", "confidence": 0.21,
          "starttime": 1234, "endtime": 1456 },
        // ...
      ]
    }
  ]
...
このPoCでは、**発話全体のconfidenceだけで「高信頼」と判断しません**。
`src/lib/amivoice/lowConfidence.ts` が token-level confidence をもとに
低信頼語句を抽出します。

- 閾値（定数。将来的に環境変数化可能）:
  - `critical`: confidence < `0.6`（`CRITICAL_CONFIDENCE_THRESHOLD`）
  - `warning` : `0.6` <= confidence < `0.75`（`LOW_CONFIDENCE_THRESHOLD`）
- 句読点・記号token（`spoken: "_"`）・助詞だけの1文字ひらがなは除外。
  ただしカタカナ・英字・数字を含む短い語は技術用語の可能性があるため残す。
- 低信頼tokenは前後のtokenを含めてphrase化（例: 「リリース」単体ではなく
  「ケースリリース」）。
- 表示は最大 `MAX_LOW_CONFIDENCE_TERMS`（8件）。critical・技術用語らしい語句を
  優先。
- **抽出した語句は正しい用語へ自動補正しません。** 目的は補正ではなく、
  Geminiのアクション判定と人間確認に回すことです（低信頼語句は
  `## AmiVoice由来の低信頼語句` としてGeminiプロンプトに渡されます）。
- AmiVoiceレスポンスにtoken単位のconfidenceが含まれない場合は、低信頼語句を
  **捏造せず空**にします（mock providerのみ、UI確認用に擬似的な語句を
  `reason` に `mock:` を付けて返します）。
